...
class searchActions extends sfActions
{
  ...
  public function executeIndex(sfWebRequest $request)
  {
    ...
    if ($this->form->isValid())
    {
      ...
      $this->getResponse()->prependTitle($this->data['nom_commune'].' ('.$this->data['code_postal'].') – Check your place');
      ...
      if (isset($this->data['InseeData'][0]))
      {
        ...
        // Pratique
        $this->data['stars']['pratique_full'] = ($this->data['stars']['access_proxi'] + $this->data['stars']['access_inter'] + $this->data['stars']['dist_travail'])/3;
        $this->data['stars']['pratique'] = round($this->data['stars']['pratique_full']);

        // Loisirs
        $this->data['stars']['loisirs_full'] = $this->data['stars']['licence_sport'];
        $this->data['stars']['loisirs'] = round($this->data['stars']['loisirs_full']);

        // Nature
        $this->data['stars']['nature_full'] = $this->data['stars']['espace_nature'];
        $this->data['stars']['nature'] = round($this->data['stars']['nature_full']);

        // Econommie
        $this->data['stars']['economie_full'] = ($this->data['stars']['emploi'] + $this->data['stars']['salaire'])/ 2;
        $this->data['stars']['economie'] = round($this->data['stars']['economie_full']);

        // sante
        $this->data['stars']['sante_full'] = ($this->data['stars']['access_doctor'] + $this->data['stars']['access_care'])/ 2;
        $this->data['stars']['sante'] = round($this->data['stars']['sante_full']);

        $this->data['stars']['all_full'] = ($this->data['stars']['pratique'] + $this->data['stars']['loisirs'] + $this->data['stars']['nature'] + $this->data['stars']['economie'] + $this->data['stars']['sante'])/ 5;
        $this->data['stars']['all'] = round($this->data['stars']['all_full']);
      }
    }
  }
